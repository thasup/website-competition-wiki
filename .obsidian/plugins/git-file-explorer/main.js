...
var WidgetManager = class {
  constructor(factory, fileExplorerHandler, basePath) {
    ...
    this.initializeRoot = async () => {
      if (!(0, import_fs.existsSync)((0, import_path.join)(this.basePath, ".git")))
        return;
      const rootEl = this.createRootWidgetContainer();
      if (!rootEl)
        return;
      const alreadyWidgetized = this.widgets.some(
        (widget) => widget.getParent().isEqualNode(rootEl)
      );
      if (alreadyWidgetized)
        return;
  ...
  async registerWidgets(parent, absPath) {
    try {
  ...
  createRootWidgetContainer() {
    const existing = document.querySelector("#git-root-widget-container");
    if (existing)
      return existing;
...
read
`;
        (0, import_fs2.writeFileSync)(tmpScriptPath, scriptContent, { mode: 493 });
        const terminals = [
          ...
          try {
            ...
            if (result.status === 0) {
              ...
              (0, import_child_process2.exec)(terminalCmd, (error) => {
                try {
                  (0, import_fs2.unlinkSync)(tmpScriptPath);
                } catch (e) {
                }
                if (error) {
...
var GitRepository = class {
  ...
  static isGitRepo(fullPath) {
    ...
    return (0, import_fs3.existsSync)(gitDir);
  }
...
// src/viewRemoteHandler.ts
var import_obsidian6 = require("obsidian");
var import_path6 = require("path");
var _ViewRemoteHandler = class {
  constructor(basePath) {
    this.basePath = basePath;
    this.buildAbsPathTo = (path2) => (0, import_path6.join)(this.basePath, path2);
  }
  async execute(fileOrFolder) {
    var _a2;
    const folderPath = fileOrFolder instanceof import_obsidian6.TFolder ? fileOrFolder.path : (_a2 = fileOrFolder.parent) == null ? void 0 : _a2.path;
    if (!folderPath)
      return;
    const absPath = this.buildAbsPathTo(folderPath);
    const repoRoot = this.findGitRepoRoot(absPath);
  ...
  getCommandName() {
    return _ViewRemoteHandler.VIEW_REMOTE;
  }
  getIcon() {
    return "external-link";
  }
  getCommandId() {
    return _ViewRemoteHandler.COMMAND_ID;
  }
  // Find the git repository root by walking up the directory tree
  findGitRepoRoot(startPath) {
    let currentPath = startPath;
    while (currentPath && currentPath.length > 0) {
      ...
      const parentPath = (0, import_path6.join)(currentPath, "..");
      if (parentPath === currentPath) {
        return null;
      }
      currentPath = parentPath;
...
var ViewRemoteHandler = _ViewRemoteHandler;
ViewRemoteHandler.VIEW_REMOTE = "View remote";
ViewRemoteHandler.COMMAND_ID = "view-remote-repo";
...
// main.ts
var GitFileExplorerPlugin = class extends import_obsidian7.Plugin {
  constructor() {
    super(...arguments);
    this.initialize = async () => {
      ...
      const capabilityProviders = [
        ...
        new InitNewRepoHandler(this.getVaultBasePath()).withCallback(() => {
          var _a2, _b;
          (_a2 = this.widgetManager) == null ? void 0 : _a2.initializeRoot();
          (_b = this.widgetManager) == null ? void 0 : _b.update();
        }),
