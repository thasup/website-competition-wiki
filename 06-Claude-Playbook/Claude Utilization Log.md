...
| 2026-07-09 | Build | Round-2 fidelity pass on the Alethia replica driven by user-captured recordings: alpha-channel canvas hand-off over a shared fixed backdrop video, blur-in copy, liquid-glass accordion, hover circle-cover cards, continuous 2-axis technology journey, radial bento explosion, closing-section DS polish | Claude Code, MCP/Browser | Verify against fresh recordings of the reference site, not memory — the cursor position in a recording revealed the lime card state was a *hover* effect, and PIL alpha-sampling of the webp tail frames revealed how the section hand-off really works |
| 2026-07-10 | Build | Re-imagined the Alethia animation mockup as Terra Digital Ventures on a `terra-rebrand` branch: asserted the DS palette/fonts/logo, kept the full home animation narrative with consulting copy, and expanded to a 5-page site (Solutions, Work, Company, Contact, Insights) with an adaptive dropdown navbar | Claude Code, Subagents, Planning | Reskin a themed site by remapping the *semantic* color tokens (forest→Viridian, lime→Cerulean, etc.) rather than editing every component — one globals.css change recolored the whole site; two Explore subagents pulled the exact DS tokens + locked copy in parallel |
| _add rows daily_ | | | | |
